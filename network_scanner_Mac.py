...
def ping_ip(ip, os_type, resolve, total, callback):
    ...
    mac = get_mac_from_arp(ip) if (status == "🟢 UP" and mac_lookup_enabled.get()) else "N/A"
    timestamp = datetime.now().strftime("%H:%M:%S")
    final = f"[{timestamp}] {display_ip} is {status} | MAC: {mac}"
...
def update_gui(result, scanned, total):
    ...
    status = result.split(" is ")[1].split(" | MAC:")[0]
...
def export_results():
    ...
    try:
        ...
        for i, line in enumerate(results, start=1):
            ...
            mac_part = "N/A"
            if " | MAC: " in status_part:
                status_split = status_part.split(" | MAC: ")
                status_part = status_split[0]
                mac_part = status_split[1]
...
mac_lookup_enabled = tk.BooleanVar(value=True)
...
tk.Checkbutton(root, text="Enable MAC Address Lookup", variable=mac_lookup_enabled,
               bg="#1e1e2e", fg="white", font=("Segoe UI", 10), selectcolor="#1e1e2e").pack(pady=0)
